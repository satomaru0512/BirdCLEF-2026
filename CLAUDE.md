...
### 重要: 新規 EXP ディレクトリの作成制限

**`EXP/EXP{exp_no}/` の新規作成は、以下のいずれかの場合のみ許可される:**
- ユーザーから明示的に作成を依頼された場合
- ユーザーから明示的に許可を得た場合

**承認なしに自律的に新しい EXP ディレクトリを作成してはならない。**

### 新規 EXP ディレクトリの作成手順

新規 `EXP/EXP{exp_no}/` を作成する際は、`EXP/EXP000/` のディレクトリ構造をテンプレートとしてコピーすること。ただし、**EXP000 固有の内容は削除・空にすること**。

- `train.py` / `infer.py`: EXP000 のファイルをベースにするが、**スクリプト本体のコード内容はそのままコピーせず**、新実験の目的に合わせて実装する（または空のテンプレートにする）
- `config/`: EXP000 の設定ファイル構造を参考にするが、中身は新実験用に書き直す
- `outputs/`: 空ディレクトリとして作成（EXP000 の出力は含めない）

**コピーしてよいもの:** ディレクトリ構造、スクリプトの骨格（import、config読み込み、wandb初期化など共通部分）
**コピーしてはいけないもの:** EXP000 固有のモデル定義・学習ロジック・推論結果・出力ファイル
...
- コードは Kaggle Notebooks で実行可能であること（推論: CPU ≤90分）
- 提出時はインターネットアクセス不可
- 事前学習済みモデルは使用可能
